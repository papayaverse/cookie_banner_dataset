...
def get_external_banner(html_source):
    ...
    keywords = ['cookie', 'cc-banner', 'tarteaucitron', 'iubenda', 'osano', 'consent', 'gdpr']
...
def click_button(driver, button_dict):
    '''
    This function takes in a dictionary of the button and clicks the button on the page
    '''
    wait = WebDriverWait(driver, 10)  # 10 seconds timeout
    if 'id' in button_dict:
        button_id = button_dict['id']
        button = wait.until(EC.element_to_be_clickable((By.ID, button_id)))
        driver.execute_script("arguments[0].click();", button)
        print(f'Clicked element ID {button_id}')
    elif 'text' in button_dict:
        button_text = button_dict['text']
        button = wait.until(EC.element_to_be_clickable((By.XPATH, f"//*[text()='{button_text}']")))
        driver.execute_script("arguments[0].click();", button)
...
def verify_button(button_dict):
    if none_or_empty(button_dict['text']) and none_or_empty(button_dict['id']) and none_or_empty(button_dict['class']):
        return False
    return True
...
def validate_buttons(driver, buttons):
    print('Validating buttons', buttons)
    # buttons is now a list of dictionaries
    new_button_list = []
    for button in buttons:
        button = validate_button(driver, button)
        if button == {}:
            continue
...
def lambda_handle_url(url, driver):
    ...
    url_ext_data_path = f'url_data_ext/{url}.json'
    url_data_path = f'url_data/{url}.json'
    url_data = {}
    url_key = url
    print("URL is ", url)
    url_data = None
    try : 
        obj = bucket_resource.Object(url_ext_data_path)
        url_data = json.loads(obj.get()['Body'].read().decode('utf-8'))
        print("Object exists in s3")
    except Exception as e :
        print("Object doesnt exist in s3")
        return {"result" : "Object does not exist in s3"}
    # get the website
    driver.get('https://www.' + url)
    # wait for stuff to load
    buttons = url_data['external_buttons']
    time.sleep(4)
    # get the html source
    if 'manage_my_preferences' in buttons:
        ...
        try :
            ...
            print(f'Error in clicking manage my preferences button {str(e)}')
            buttons.pop('manage_my_preferences')
            url_data['external_buttons'] = buttons
            bucket_resource.put_object(Key=url_data_path, Body=json.dumps(url_data))
            return {"result" : "No manage my preferences button"}
        # wait for stuff to load
        time.sleep(4)
        # get the html source
        html_source = driver.page_source
        # get the internal banner
        internal_banner = get_internal_banner(html_source)
        if internal_banner is not None:
            # store the internal banner text
            url_data['internal_texts'] = internal_banner.text
            # get the internal buttons
            internal_buttons = {}
            try :
                # get the internal buttons
                internal_buttons = get_internal_buttons(internal_banner)
                print('Internal buttons', internal_buttons)
                # store the internal buttons
                url_data['internal_buttons'] = internal_buttons#validate_buttons(driver, internal_buttons)
                print('Internal buttons stored', url_data['internal_buttons'])
            except Exception as e:
                print(f'Error in getting internal buttons {str(e)}')
    bucket_resource.put_object(Key=url_data_path, Body=json.dumps(url_data))
    #driver.close()
    return {"result" : "Success", "url_data_path" : url_data_path}

def lambda_handler(event, context):
    folder_path = 'url_data_ext/'
    bucket_name = 'cookie-monster-bucket-rammkripa'
    response = s3.list_objects_v2(Bucket=bucket_name, Prefix=folder_path)
    urls = []
    for obj in response['Contents']:
        url_key = re.findall(r'^url_data_ext/(.*).json$',obj['Key'])[0]
        urls.append(url_key)
    urls_in_event = event['urls']
    urls_intersection = list(set(urls).intersection(set(urls_in_event)))
    count_of_success = 0
    count_of_error = 0
    count_total = len(urls_intersection)
    # start the driver
    driver = launch_browser()
    driver.get('https://www.selenium.dev/')
    for url in urls_intersection:
        try :
